...
def group_containers(df):
    backend_group = df[df['Container Name'].str.startswith('backend')]
    frontend_group = df[df['Container Name'].str.startswith('frontend')]
    worker_group = df[df['Container Name'].str.startswith('worker')]
    other_group = df[~df['Container Name'].str.startswith(('backend', 'frontend', 'worker'))]

    return backend_group, frontend_group, worker_group, other_group
...
def generate_html(backend_img, frontend_img, worker_img, other_img):
    html = f"""
    <html>
    ...
    <h2>Venom Group</h2>
    <img src="data:image/png;base64,{backend_img}" alt="Venom Group Graphs">

    <h2>Natrix Group</h2>
    <img src="data:image/png;base64,{frontend_img}" alt="Natrix Group Graphs">

    <h2>Audit Group</h2>
    <img src="data:image/png;base64,{worker_img}" alt="Audit Group Graphs">
...
def plot_from_csv(csv_file='container_stats.csv'):
    ...
    backend_group, frontend_group, worker_group, other_group = group_containers(df)

    # Создаем графики для каждой группы
    backend_img = create_plot(backend_group, 'Venom Group')
    frontend_img = create_plot(frontend_group, 'Natrix Group')
    worker_img = create_plot(worker_group, 'Audit Group')
    other_img = create_plot(other_group, 'Other Group')

    # Генерируем HTML
    generate_html(backend_img, frontend_img, worker_img, other_img)
